...
class MultiHeadSGATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_heads, attn_drop_out, merge='cat'):
        ...
        for i in range(num_heads):
            self.heads.append(               
                SGATLayer(in_dim, out_dim))  
        self.merge = merge
        self.dropout = nn.Dropout(attn_drop_out)

    def forward(self, g, h):
        head_outs = [attn_head(g, self.dropout(h)) for attn_head in self.heads]  
        if self.merge == 'cat':
...
class MultiHeadLayer(nn.Module):
    ...
    def forward(self, g, h, k):
        head_outs = [attn_head(g, self.dropout(h), self.dropout(k)) for attn_head in self.heads]  
        if self.merge == 'cat':
